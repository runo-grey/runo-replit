...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName("rank")
    .setDescription("Show your XP rank card with level and progress")
    .addUserOption(opt =>
      opt.setName("user").setDescription("Check another user's rank"),
    ),
  ...
  new SlashCommandBuilder()
    .setName("set-automod")
    .setDescription("Admin: set the channel where automod logs are sent")
    .addChannelOption(opt =>
      opt.setName("channel").setDescription("Channel to send automod logs to").setRequired(true),
    ),

  new SlashCommandBuilder()
    .setName("whitelist")
    .setDescription("Admin: exempt a user from AutoMod — their messages will never be flagged")
    .addUserOption(opt =>
      opt.setName("user").setDescription("User to whitelist from AutoMod").setRequired(true),
    ),

  new SlashCommandBuilder()
    .setName("set-audit-logs")
    .setDescription("Admin: set the channel where audit logs (deleted/edited messages) are sent")
    .addChannelOption(opt =>
      opt.setName("channel").setDescription("Channel to send audit logs to").setRequired(true),
    ),
